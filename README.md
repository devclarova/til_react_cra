...
- node_modules에 다운로드 받아서 파일을 관리하는 용도
...
- public 폴더는 `npm run build` 시 `압축 제외`
- 우리가 필요로 한 `images, fonts와 같은 리소스를 배치`함
- favicon.ico: 즐겨찾기, 주소 공유 시 보여질 아이콘
- logo192.png, logo512,png: 휴대폰에서 바로가기 저장 시 보여질 아이콘
- mainifest.json: 리액트도 웹 앱이라서 앱의 설명서(PWA)
- robots.txt: 웹 크롤링 여부, 검색엔진 탐색 허용 여부
- sitemap.xml: 생성 권장 (검색엔진 노출 위해)
- index.html: 웹 실행 시 최초로 보여질 페이지
...
<html lang="ko">
  <head>
    ...
    <script defer="defer" src="/static/js/main.15d342e7.js"></script>
...
// js로 root에 html React 명령으로 그려라
root.render(
  // React.StructMode는 개발 메세지 여러번 출력하라
  // <React.StrictMode>
  <App />,
  // </React.StrictMode>
);
...
- App.js: 최초로 보여주는 내용 컴포넌트 (html, css, js 한 파일에)
