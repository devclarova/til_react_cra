...
# useEffect와 useState, Event 종합 예제
...
function Todo() {
  ...
  const handleDeleteTodo = id => {
    // todoList 목록에서 id와 같은 것을 찾고 제거하고 목록 업데이트
      ...
      <Section>
        {todoList.map(item => (
          <TodoItem key={item.id}>
            ...
            <TodoButtonWrap>
              <Button onClick={e => handleDeleteTodo(item.id)}>삭제</Button>
              <Button>수정</Button>
            </TodoButtonWrap>
          </TodoItem>
        ))}
...
function Todo() {
  ...
  const handleTodoListSelect = id => {
    // 만약 id만 전달한다면 todoList에서 데이터를 뽑는 작업을 다시 진행
...
- Session: 웹브라우저에서 활용하는 동안 유지(웹브라우저 종료 시 파기)
- Local Storage: 웹브라우저에 짧은 문자열 보관 (가장 흔함)

### 6.2. Local Storage

- 시나리오 1: 웹서비스가 시작하면 `할일 내용` 가져옮
...
function Todo() {
  ...
  // 컴포넌트에서 데이터 관리
  // 처음에 컴포넌트 화면에 보일 때 한 번만 자료 불러오기
  useEffect(() => {
    // 초기값 불러오기
    const result = localStorage.getItem("todolist");
    if (!result) {
      // 결과가 없다면 다시 처음으로 세팅한다.
